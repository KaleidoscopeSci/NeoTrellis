...
SAMPLE_FOLDER = "/nature/"  # the name of the folder containing the samples
# You get 4 voices, they must all have the same sample rate and must
# all be mono or stereo (no mix-n-match!)
VOICES = [SAMPLE_FOLDER+"wav_bird20.wav",
          SAMPLE_FOLDER+"wav_bug3.wav",
          SAMPLE_FOLDER+"wav_frogs1.wav",
          SAMPLE_FOLDER+"wav_mammal1.wav",
          SAMPLE_FOLDER+"wav_other1.wav",
          SAMPLE_FOLDER+"wav_other2.wav",
          SAMPLE_FOLDER+"wav_weather1.wav",
          SAMPLE_FOLDER+"wav_weather2.wav"]
...
with open(VOICES[0], "rb") as f:
    ...
    mixer = audioio.Mixer(voice_count=8,
...
samples = []
# Read the 8 wave files, convert to stereo samples, and store
# (show load status on neopixels and play audio once loaded too!)
for v in range(8):
    trellis.pixels[(0, v)] = DRUM_COLOR[v]
    wave_file = open(VOICES[v], "rb")
    # OK we managed to open the wave OK
    for x in range(1, 4):
        trellis.pixels[(x, v)] = DRUM_COLOR[v]
...
#        trellis.pixels[(x, v)] = DRUM_COLOR[v]
    while mixer.playing:
        pass
    trellis.pixels[(3, v)] = DRUM_COLOR[v]
...
beatset = [[False] * 4, [False] * 4, [False] * 4, [False] * 4,[False] * 4, [False] * 4, [False] * 4, [False] * 4]
...
while True:
    ...
    for y in range(8):
        ...
        if beatset[y][current_step]:
            color = DRUM_COLOR[y]
        trellis.pixels[(current_step, y)] = color
    ...
    for y in range(8):
        if beatset[y][current_step]:
            ...
            #print("Playing: ", VOICES[y]) THis next line does not work!
    ...
    while time.monotonic() - stamp < 60/tempo:
        ...
        if ENABLE_TILT_TEMPO:
            ...
            tilt = accelerometer.acceleration[0]
            #print("%0.1f" % tilt)
            new_tempo = tempo
            if tilt < -9:
                new_tempo = tempo + 5
